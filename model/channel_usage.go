...
type channelTokenUsageRow struct {
	...
	UsedQuotaToday  int64 `gorm:"column:used_quota_today"`
}
...
func PopulateChannelTokenUsage(channels []*Channel) error {
	...
	err := LOG_DB.Model(&Log{}).
		Select(
			"channel_id, "+
				...
				"COALESCE(SUM(CASE WHEN created_at >= ? THEN quota ELSE 0 END), 0) AS used_quota_today",
			startOfToday,
			startOfToday,
			startOfToday,
		).
	...
	for _, channel := range channels {
		...
		if row, ok := usageByChannelID[channel.Id]; ok {
			...
			channel.UsedQuotaToday = row.UsedQuotaToday
		}
	}
