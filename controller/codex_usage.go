...
func GetCodexChannelUsage(c *gin.Context) {
	...
	client, err := service.NewProxyHttpClient(ch.GetSetting().GetProxyURL())
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	if (statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden) && strings.TrimSpace(oauthKey.RefreshToken) != "" {
		...
		res, refreshErr := service.RefreshCodexOAuthTokenWithProxy(refreshCtx, oauthKey.RefreshToken, ch.GetSetting().GetProxyURL())
		if refreshErr == nil {
